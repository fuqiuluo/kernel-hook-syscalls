...
uintptr_t *find_syscall_table(void) {
    syscall_table = (uintptr_t*)my_kallsyms_lookup_name("sys_call_table");
    return syscall_table;
}

uintptr_t my_kallsyms_lookup_name(const char* symbol_name) {
#if LINUX_VERSION_CODE >= KERNEL_VERSION(5, 7, 0) \
    // 大于这个版本的内核已经没有kallsyms_lookup_name导出了
    struct kprobe kp = {
        .symbol_name = "kallsyms_lookup_name"
    };

    typedef uintptr_t (*kallsyms_lookup_name_t)(const char *name);
